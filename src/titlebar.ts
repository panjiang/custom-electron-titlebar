...
const defaultOptions = (win: BrowserWindow): TitlebarOptions => {
  return {
    backgroundColor: Color.fromHex('#444444'),
    iconsTheme: Themebar.win,
    shadow: false,
    menu: remote.Menu.getApplicationMenu(),
    minimizable: win.hasOwnProperty('minimizable') ? win.minimizable : win.isMinimizable(),
    maximizable: win.hasOwnProperty('maximizable') ? win.maximizable : win.isMaximizable(),
    closeable: win.hasOwnProperty('closable') ? win.closable : win.isClosable(),
...
export class Titlebar extends Themebar {
  ...
  constructor(options?: TitlebarOptions) {
    ...
    this._options = {
      ...defaultOptions(this.currentWindow),
      ...options,
    };
  ...
  private createTitlebar() {
    ...
    if (this._options.menuPosition === 'bottom') {
      ...
      this.container.style.top = isMacintosh
    ...
    if (!isMacintosh) {
      ...
      // Minimize
      const minimizeIconContainer = append(
        this.windowControls,
        $('div.window-icon-bg')
      );
      ...
      // Restore
      const restoreIconContainer = append(
        this.windowControls,
        $('div.window-icon-bg')
      );
      ...
      if (!this._options.maximizable) {
        ...
        this._register(
          addDisposableListener(this.maxRestoreControl, EventType.CLICK, e => {
      ...
      // Close
      const closeIconContainer = append(
        this.windowControls,
        $('div.window-icon-bg')
      );
  ...
  private updateStyles() {
    if (this.titlebar) {
      ...
      const titleBackground = this.isInactive
        ? this._options.backgroundColor.lighten(0.3)
      ...
      if (titleBackground.isLighter()) {
        ...
        const titleForeground = this.isInactive
      ...
      const backgroundColor = this._options.backgroundColor.darken(0.1);
      const foregroundColor = backgroundColor.isLighter()
        ? INACTIVE_FOREGROUND_DARK
        : INACTIVE_FOREGROUND;
      const bgColor =
        !this._options.itemBackgroundColor ||
        this._options.itemBackgroundColor.equals(backgroundColor)
          ? new Color(new RGBA(0, 0, 0, 0.14))
          : this._options.itemBackgroundColor;
      const fgColor = bgColor.isLighter()
        ? ACTIVE_FOREGROUND_DARK
        : ACTIVE_FOREGROUND;
  ...
  updateMenuPosition(menuPosition: 'left' | 'bottom') {
    ...
    if (isMacintosh) {
      this.titlebar.style.height =
        this._options.menuPosition && this._options.menuPosition === 'bottom'
    ...
    this.titlebar.style.webkitFlexWrap =
      this._options.menuPosition && this._options.menuPosition === 'bottom'
        ? 'wrap'
        : null;
  ...
  setHorizontalAlignment(side: 'left' | 'center' | 'right') {
    if (this.title) {
      if (
        side === 'left' ||
        (side === 'right' && this._options.order === 'inverted')
      ) {
      ...
      if (
        side === 'right' ||
        (side === 'left' && this._options.order === 'inverted')
      ) {
