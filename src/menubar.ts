...
export class Menubar extends Disposable {
  ...
  setupMenubar(): void {
    const topLevelMenus = (this.options.menu
      .items as unknown) as MenuItemConstructorOptions[];
    ...
    topLevelMenus.forEach(menubarMenu => {
      ...
      // Register mnemonics
      if (mnemonicMatches) {
        let mnemonic = !!mnemonicMatches[1]
          ? mnemonicMatches[1]
          : mnemonicMatches[2];
  ...
  private onClick(menuIndex: number) {
    ...
    if (item.click) {
      item.click(
        (item as unknown) as Electron.MenuItem,
  ...
  private focusPrevious(): void {
    ...
    let newFocusedIndex =
      (this.focusedMenu.index - 1 + this.menuItems.length) %
  ...
  private showMenu(menuIndex: number, selectFirst = true): void {
    ...
    menuHolder.style.left = `${
      customMenu.buttonElement.getBoundingClientRect().left
    }px`;
    ...
    menuWidget.createMenu((customMenu.submenu.items as unknown) as IMenuItem[]);
