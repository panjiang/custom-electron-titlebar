  ...
  createMenu(items: IMenuItem[]) {
    items.forEach((menuItem: IMenuItem) => {
      ...
      if (menuItem.type === "separator") {
        ...
        const submenuItems = ((menuItem.submenu as Electron.Menu)
          .items as unknown) as IMenuItem[];
        item = new Submenu(
          menuItem,
          submenuItems,
          this.parentData,
...
class Submenu extends MenuItem {
  ...
  constructor(
    ...
    this.hideScheduler = new RunOnceScheduler(() => {
      if (
        this.container &&
        (!isAncestor(document.activeElement, this.container) &&
  ...
  private createSubmenu(selectFirstItem = true): void {
    ...
    if (!this.parentData.submenu) {
      ...
      const computedStyles = getComputedStyle(
        this.parentData.parent.getContainer()
      );
      const paddingTop = parseFloat(computedStyles.paddingTop || "0") || 0;

      if (window.innerWidth <= boundingRect.right + childBoundingRect.width) {
        this.submenuContainer.style.left = "10px";
        this.submenuContainer.style.top = `${this.container.offsetTop +
