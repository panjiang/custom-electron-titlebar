...
export class MenuItem extends Disposable implements IMenuItem {
  ...
  constructor(
    ...
    if (this.item.label && options.enableMnemonics) {
      ...
      if (label) {
        ...
        if (matches) {
          this.mnemonic = KeyCodeUtils.fromString(
            (!!matches[1] ? matches[1] : matches[2]).toLocaleUpperCase()
          );
        }
      }
    }
  ...
  render(container: HTMLElement): void {
    ...
    this._register(
      addDisposableListener(this.container, EventType.MOUSE_DOWN, e => {
    ...
    this._register(
      addDisposableListener(this.container, EventType.DBLCLICK, e => {
    ...
    [EventType.MOUSE_UP, EventType.MOUSE_OUT].forEach(event => {
      this._register(
        addDisposableListener(this.container!, event, e => {
          EventHelper.stop(e);
  ...
  onClick(event: EventLike) {
    ...
    if (this.item.click) {
      this.item.click(
        (this.item as unknown) as Electron.MenuItem,
        this.currentWindow,
        this.event
      );
    }
  ...
  setAccelerator(): void {
    ...
    if (accelerator !== null) {
      append(
        this.itemElement,
        $("span.keybinding")
      ).textContent = parseAccelerator(accelerator);
    }
  }
      ...
      if (label) {
        ...
        if (matches) {
          ...
          this.itemElement.setAttribute(
            "aria-keyshortcuts",
            (!!matches[1] ? matches[1] : matches[2]).toLocaleLowerCase()
          );
        }
      }
  ...
  protected applyStyle() {
    ...
    this.checkElement.style.backgroundColor = fgColor
      ? fgColor.toString()
      : null;
    this.itemElement.style.color = fgColor ? fgColor.toString() : null;
    this.itemElement.style.backgroundColor = bgColor
      ? bgColor.toString()
      : null;
  }
